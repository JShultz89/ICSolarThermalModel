...
def R(interface, T):
    if interface == "wa":  #water to air
        ...
        R_conv_wt = 1/(2* math.pi * (3*10**-3) * 0.3 * h('p',(90+13)/2))    # Convection from water to tubing
        R_cond_t = math.log(((3+1.5)*10**-3)/((3)*10**-3)) / (2 * math.pi * (3+1.5)*10**-3 * 0.145 * 0.3)    # Conduction through tubing
        R_cond_ins = math.log(((3+1.5+ins_pipe)*10**-3)/((3+1.5)*10**-3)) / (2 * math.pi * (3+1.5+ins_pipe)*10**-3 * 0.037 * 0.3)    # Conduction through insulation
        R_conv_ta = 1/(2 * math.pi * (3+1.5+ins_pipe)*10**-3 * 0.3 * h('pa',(30+13)/2))    # Convenction from tubing to air
...
def Differentiation(q,T):
   ...
   dR1dT0 = m_w * Cp('w', ((T[j+2]-T[j])/2)*(T[j+2]-0)) - ( ((T[j+3]-T[j+1])/2.0-(T[j+2]-0)/2.0) / R('wa',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2) )

   # Derivative of R1 (water balance) with respect to T[j+1] (T[1])
   dR1dT1 = m_w * Cp('w', ((T[j+2]-T[j])/2)*(T[j+2]-T[j])) - ( ((T[j+3]-0)/2.0-(T[j+2]-T[j])/2.0) / R('wa',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2) )

   # Derivative of R1 (water balance) with respect to T[j+2] (T[2])
   dR1dT2 = m_w * Cp('w', ((T[j+2]-T[j])/2)*(0-T[j])) - ( ((T[j+3]-T[j+1])/2.0-(0-T[j])/2.0) / R('wa',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2) )

   # Derivative of R1 (water balance) with respect to T[j+3] (T[3])
   dR1dT3 = m_w * Cp('w', ((T[j+2]-T[j])/2)*(T[j+2]-T[j])) - ( ((0-T[j+1])/2.0-(T[j+2]-T[j])/2.0) / R('wa',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2) )

   # Region 2: Heat Balance in the Air
   # Derivative of R2 (air balance) with respect to T[j] (T[0])
   dR2dT0 = m_a * Cp('a',(T[j+3]-T[j+1])/2) * (T[j+3]-T[j+1]) + (((T[j+3]-T[j+1])/2.0 - (T[j+2]-0)/2.0)/R('wa',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2.0)) - (T_int-(T[j+3]-T[j+1])/2.0)/R('i',T_int-(T[j+3]-T[j+1])/2) - (T_ext-(T[j+3]-T[j+1])/2.0)/R('e',T_ext-(T[j+3]-T[j+1])/2)

   # Derivative of R2 (air balance) with respect to T[j+1] (T[1])
   dR2dT1 = m_a * Cp('a',(T[j+3]-T[j+1])/2) * (T[j+3]-0) + (((T[j+3]-0)/2.0 - (T[j+2]-T[j])/2.0)/R('wa',(T[j+3]-T[j+1])/2.0 - (T[j+2]-T[j])/2.0)) - (T_int-(T[j+3]-0)/2.0)/R('i',T_int-(T[j+3]-T[j+1])/2) - (T_ext-(T[j+3]-0)/2.0)/R('e',T_ext-(T[j+3]-T[j+1])/2)

   # Derivative of R2 (air balance) with respect to T[j+2] (T[2])
   dR2dT2 = m_a * Cp('a',(T[j+3]-T[j+1])/2) * (T[j+3]-T[j+1]) + (((T[j+3]-T[j+1])/2.0 - (0-T[j])/2.0)/R('wa',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2.0)) - (T_int-(T[j+3]-T[j+1])/2.0)/R('i',T_int-(T[j+3]-T[j+1])/2) - (T_ext-(T[j+3]-T[j+1])/2.0)/R('e',T_ext-(T[j+3]-T[j+1])/2)

   # Derivative of R2 (air balance) with respect to T[j+3] (T[3])
   dR2dT3 = m_a * Cp('a',(T[j+3]-T[j+1])/2) * (0-T[j+1]) + (((0-T[j+1])/2.0 - (T[j+2]-T[j])/2.0)/R('wa',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2.0)) - (T_int-(0-T[j+1])/2.0)/R('i',T_int-(T[j+3]-T[j+1])/2) - (T_ext-(0-T[j+1])/2.0)/R('e',T_ext-(T[j+3]-T[j+1])/2)
...
m_a = 2.0*10**(-6) * rho_a((13+30)/2) # Mass Flowrate of air = VolumetricFlowrate * DensityAir
