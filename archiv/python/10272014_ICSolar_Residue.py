...
def Cp(liquid, T):
    ...
    if liquid == "w":
        ...
        p1 =   -0.001201  (-0.001769, -0.000633)
        p2 =     0.00218  (0.001711, 0.002649)
        p3 =    0.001323  (-0.000498, 0.003144)
        p4 =     0.00634  (0.005152, 0.007527)
        p5 =     0.01152  (0.01022, 0.01281)
        p6 =       4.183  (4.183, 4.184)
...
def rho_w(T):
    p1 =   -0.003416  (-0.003623, -0.003209)
    p2 =    -0.09298  (-0.1165, -0.06949)
    p3 =        1001  (1001, 1002)
    rho = p1*T**2 + p2*T + p3
    return rho
...
def h(interface,T):
    ...
    def k(liquid, T):
        if liquid == "w":
            ...
            # R = 1
            p1 =       7e-05  (7e-05, 7e-05)
            p2 =      0.0243  (0.0243, 0.0243)
            k = p1*T + p2
    ...
    def Pr_a(T):
        p1 =  -4.705e-19  (-7.3e-18, 6.359e-18)
        p2 =     -0.0001  (-0.0001, -0.0001)
        p3 =       0.715  (0.715, 0.715)
        Pr = p1*T**2 + p2*T + p3
        return Pr
...
def Residue(m,T):
    ...
    for i in range(0,m):
        ...
        q_w1 = m_w * Cp('w',(T[j+2]-T[j])/2) * (T[j+2]-T[j]) - h('p',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2) * As_pipe * ((T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2.0)
        q[j] = q_w1
        # Calculate air temperature for region 1 (Transfer with water, interior and exterior)
        q_a1 = m_a * Cp('a',(T[j+3]-T[j+1])/2) * (T[j+3]-T[j+1]) + h('pa',(T[j+3]-T[j+1])/2.0-(T[j+3]-T[j+1])/2.0) * As_pipe * ((T[j+3]-T[j+1])/2.0 - (T[j+2]-T[j])/2.0) - h('i',T_i-(T[j+3]-T[j+1])/2) * A * (T_i-(T[j+3]-T[j+1])/2.0) - h('e',T_e-(T[j+3]-T[j+1])/2) * A * (T_e-(T[j+3]-T[j+1])/2.0)
        q[j+1] = q_a1
