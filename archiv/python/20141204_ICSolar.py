...
def R(interface, T):
    if interface == "wa":  #water to air
        ...
        R_cond_t = math.log(((3+1.5)*10**-3)/((3)*10**-3)) / (2 * math.pi * (3+1.5)*10**-3 * 0.3 * 0.145)    # Conduction through tubing
        R_cond_ins = math.log(((3+1.5+ins_pipe)*10**-3)/((3+1.5)*10**-3)) / (2.0 * math.pi * (3+1.5+ins_pipe)*10**-3 * 0.3 * 0.037)    # Conduction through insulation
...
def Differentiation(q,T):
    ...
    for i in range(0,m):
        ...
        dR1dT2_a = -1.0/2.0/R('wa',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2.0)
        dRdT[j+1,j] = dR1dT2_a
...
m_a = v_a * rho_a((13+30)/2) * 0.4*0.4 # Mass Flowrate of air [kg/s] = velocity * DensityAir * cross section
