...
def h(interface, T): 
    ...
    def viscosity_a(T): #Kinematic Viscosity
        p1 =     7.5e-05
        p2 =      0.0888
        p3 =        13.3
    ...
    if mode == 'forced':
        ...
        if interface == 'p':
            ...
            Re  = v_a * 0.3*m / viscosity_a(T) # Re = velocity*Length/viscosity
...
def Residue(m,T):
    ...
    for i in range(0,m):
        ...
        q_a1 = m_a * Cp('a',(T[j+3]-T[j+1])/2) * (T[j+3]-T[j+1]) + (((T[j+3]-T[j+1])/2.0 - (T[j+2]-T[j])/2.0)/R('wa',(T[j+3]-T[j+1])/2.0-(T[j+3]-T[j+1])/2.0)) - (T_i-(T[j+3]-T[j+1])/2.0)/R('i',T_i-(T[j+3]-T[j+1])/2) - (T_e-(T[j+3]-T[j+1])/2.0)/R('e',T_e-(T[j+3]-T[j+1])/2)
        q[j+1] = q_a1
...
radius_OD = 3+1.5+ins_pipe # Outside Diameter Radius of Pipe with Insulation
As_pipe = 2 * math.pi * ((radius_OD)*10**-3) * 0.3 # Area m^2
