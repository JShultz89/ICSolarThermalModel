...
def h(interface, T): 
    ...
    if mode == 'forced':
        ...
        if interface == 'w':
            ...
            Re  = v_a * 0.3*m / viscosity_a(T) # Re = velocity*Desnity*Length/viscosity
            Nu = 0.037*Re**(4.0/5.0)*Pr_a(T)**(1.0/3.0) 
        elif interface == 'i':
            k = k('a', T)
            L = 0.3*m
            Re = v_a * 0.3*m / viscosity_a(T)
            Nu = 0.037*Re**(4.0/5.0)*Pr_a(T)**(1.0/3.0)
        elif interface == 'e':
            k = k('a', T)
            L = 0.3*m
...
def GenTemperatureArray(m):
    ...
    while count < 4*m+2:
        if count%2 == 0:
            ...
            T_w_tmp += 0.085547493106171 # Increase each air temperature by 5 degrees 
        else:
            T_tmp = T_a_tmp
            T[count] = T_tmp # Add the first water temeperature to the end of the array
            T_a_tmp += -10.060751191691518 # Increase each water temperature by 5 degrees
        count += 1
    return T
    #    print T
...
def Residue(m,T):
    ...
    for i in range(0,m):
        ...
        q_a1 = m_a * Cp('a',(T[j+3]+T[j+1])/2) * (T[j+3]-T[j+1]) + (((T[j+3]+T[j+1])/2.0 - (T[j+2]+T[j])/2.0)/R('pipe')) - (T_int-(T[j+3]+T[j+1])/2.0)/R('int') - (T_ext-(T[j+3]+T[j+1])/2.0)/R('ext')
        q[j+1] = q_a1
...
def Differentiation(q,T):
    ...
    for i in range(0,m):
        ...
        dR1dT0_w = - m_w * Cp('w', ((T[j+2]-T[j])/2)) + 1.0/2.0 / R('pipe',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2.0)
        if j == 0:
            #Do nothing
            pass
        else:
        ...
        dR1dT1_w = 1.0/2.0 / R('pipe',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2)
        if j == 0:
            #Do nothing
            pass
        else:
        ...
        dR1dT2_w = m_w * Cp('w', ((T[j+2]-T[j])/2)) + 1.0/2.0 / R('pipe',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2.0)
        dRdT[j,j] = dR1dT2_w
    
        # Derivative of R1 (water balance) with respect to T[j+3] (T[3]) 
        dR1dT3_w = - 1.0/2.0 / R('pipe',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2)
        dRdT[j,j+1] = dR1dT3_w
        ...
        dR1dT0_a = -1.0/2.0/R('pipe',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2.0)
        if j == 0:
            #Do nothing
            pass
        else:
        ...
        dR1dT1_a = - m_a * Cp('a',(T[j+3]-T[j+1])/2) + 1.0/2.0/R('pipe',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2.0) + 1.0/2.0/R('int',T_int-(T[j+3]-T[j+1])/2.0) + 1.0/2.0/R('ext',T_ext-(T[j+3]-T[j+1])/2)
        if j == 0:
            #Do nothing
            pass
        else:
        ...
        dR1dT2_a = -1.0/2.0/R('pipe',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2.0)
        dRdT[j+1,j] = dR1dT2_a
    
        # Derivative of R1 (air balance) with respect to T[j+3] (T[3])
        dR1dT3_a = m_a * Cp('a',(T[j+3]-T[j+1])/2) + 1.0/2.0/R('pipe',(T[j+3]-T[j+1])/2.0-(T[j+2]-T[j])/2.0) + 1.0/2.0/R('int',T_int-(T[j+3]-T[j+1])/2) + 1.0/2.0/R('ext',T_ext-(T[j+3]-T[j+1])/2.0)
...
diameter_inside = 3*10**(-3)
diameter_tubing = diameter_inside + 1.5*10**(-3)
ins_thickness = 19.05*10**(-3) # Thickness of the insulation around the silicone pipes
