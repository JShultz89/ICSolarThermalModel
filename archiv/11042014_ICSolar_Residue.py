...
def h(interface, T): 
    ...
    if mode == 'forced':
        ...
        if interface == 'p':
            ...
            Re  = rho_a(T) * m_a * radius_OD / viscosity_a(T) # Update to use length Re_L from HW_6
...
def R(interface,T):
    if interface == "wa":  #water to air
        ...
        R_conv_wt = 1/(2* math.pi * (3*10**-3) * 0.3 * h('p',T))    # Convection from water to tubing
        ...
        R_conv_ta = 1/(2 * math.pi * (3+1.5+ins_pipe)*10**-3 * 0.3 * h('pa',T))    # Convenction from tubing to air
...
m_a = 2.0*10**(-6) * rho_a((13+90)/2) # Mass Flowrate of air = VolumetricFlowrate * DensityAir
